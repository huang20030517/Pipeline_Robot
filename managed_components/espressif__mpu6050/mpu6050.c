...
{
    static float yaw_bias = 0; // 初始化 yaw 偏移
    float acce_angle[2];
    float gyro_angle[2];
    mpu6050_dev_t *sens = (mpu6050_dev_t *)sensor;
    ...
    if (sens->counter == 0)
    {
        ...
        complimentary_angle->yaw = 0; // 初始化 yaw 角度为 0

        yaw_bias = gyro_value->gyro_z; // 记录初始偏移
    ...
    sens->dt = (float)dt_t.tv_sec + (float)dt_t.tv_usec / 1000000.0f;
    gettimeofday(sens->timer, NULL);

    // 加速度计角度计算（roll 和 pitch）
    acce_angle[0] = atan2(acce_value->acce_y, acce_value->acce_z) * RAD_TO_DEG;
    acce_angle[1] = atan2(acce_value->acce_x, acce_value->acce_z) * RAD_TO_DEG;

    // 陀螺仪角度增量计算（roll 和 pitch）
    gyro_angle[0] = gyro_value->gyro_x * sens->dt;
    gyro_angle[1] = gyro_value->gyro_y * sens->dt;

    // 更新 roll 和 pitch，通过互补滤波融合加速度计和陀螺仪数据
    complimentary_angle->roll = (ALPHA * (complimentary_angle->roll + gyro_angle[0])) + ((1.0f - ALPHA) * acce_angle[0]);
    complimentary_angle->pitch = (ALPHA * (complimentary_angle->pitch + gyro_angle[1])) + ((1.0f - ALPHA) * acce_angle[1]);

    // 更新 yaw：使用 z轴角速度减去初始偏移
    complimentary_angle->yaw += (gyro_value->gyro_z - yaw_bias) * sens->dt;

    // 可设置一个最大漂移角度范围
    if (complimentary_angle->yaw > 180.0f)
        complimentary_angle->yaw -= 360.0f;
    else if (complimentary_angle->yaw < -180.0f)
        complimentary_angle->yaw += 360.0f;
    return ESP_OK;
}
